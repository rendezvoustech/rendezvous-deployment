...
while [ $(kubectl get pod -l app.kubernetes.io/name=rendezvous-rabbitmq -n rabbitmq-system | wc -l) -eq 0 ] ; do
  sleep 15
done
...
kubectl wait \
  --for=condition=ready pod \
  --selector=app.kubernetes.io/name=rendezvous-rabbitmq \
  --timeout=600s \
  --namespace=rabbitmq-system
...
export RABBITMQ_USERNAME=$(kubectl get secret rendezvous-rabbitmq-default-user -o jsonpath='{.data.username}' -n=rabbitmq-system | base64 --decode)
export RABBITMQ_PASSWORD=$(kubectl get secret rendezvous-rabbitmq-default-user -o jsonpath='{.data.password}' -n=rabbitmq-system | base64 --decode)
...
kubectl delete secret rendezvous-rabbitmq-credentials || true

kubectl create secret generic rendezvous-rabbitmq-credentials \
    --from-literal=spring.rabbitmq.host=rendezvous-rabbitmq.rabbitmq-system.svc.cluster.local \
...
echo "\n🍃 Secret 'rendezvous-rabbitmq-credentials' has been created for Spring Boot applications to interact with RabbitMQ."
